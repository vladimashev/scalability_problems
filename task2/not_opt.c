...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < size; ++i)
    {
        if (rank == 0)
        {
            MPI_Send(sum,  matrix_size[0] * matrix_size[1], MPI_DOUBLE, (rank + 1) % size, 0, MPI_COMM_WORLD);
            MPI_Recv(buf, matrix_size[0] * matrix_size[1], MPI_DOUBLE, size - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        } else if (rank % 2 == 0)
        {
            MPI_Send(sum,  matrix_size[0] * matrix_size[1], MPI_DOUBLE, (rank + 1) % size, 0, MPI_COMM_WORLD);
            MPI_Recv(buf, matrix_size[0] * matrix_size[1], MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        else
        {
            MPI_Recv(buf, matrix_size[0] * matrix_size[1], MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
